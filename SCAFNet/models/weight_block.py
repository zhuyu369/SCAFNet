...
#         for k in range(grey_level):
#             if p[k] != 0:
#                 entropies[i] += -p[k] * np.log2(p[k])
#         counter.fill(0)  # 重置计数器以便下一次使用
#
#     return entropies
...
def piqe(im):
    ...
    if n_dim == 4:
        n_im = len(im)
        im_np = make_grid(im, nrow=int(n_im / 3), normalize=False).cpu().numpy()
        im = np.transpose(im_np, (1, 2, 0))
...
def weight_block(input1, input2):
    c = 15
    en1 = EN(input1)
    en2 = EN(input2)
    piqe1, _, _, _ = piqe(input1)
    piqe2, _, _, _ = piqe(input2)
    sco1 = piqe1 + en1[0][0]
    sco2 = piqe2 + en2[0][0]
